...
function StepIndicator({ current }: { current: Step }) {
  return (
    <div className="flex items-center gap-2 justify-center">
      ...
      <div className="flex items-center gap-1.5">
        <div className={`w-6 h-6 rounded-full text-xs font-semibold flex items-center justify-center ${current > 2 ? "bg-muted text-muted-foreground" : "bg-foreground text-background"}`}>
          {current > 2
            ? <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            : "2"
          }
        </div>
        <span className={`text-xs ${current === 2 ? "font-medium" : "text-muted-foreground"}`}>Workspace</span>
      </div>
      <div className="h-px w-8 bg-foreground/30" />
      <div className="flex items-center gap-1.5">
        <div className={`w-6 h-6 rounded-full text-xs font-semibold flex items-center justify-center ${current === 3 ? "bg-foreground text-background" : "bg-muted text-muted-foreground"}`}>
          3
        </div>
        <span className={`text-xs ${current === 3 ? "font-medium" : "text-muted-foreground"}`}>Invite team</span>
      </div>
    </div>
  )
}
